document.addEventListener('DOMContentLoaded', function() {
  ...
  let celebrationType = "birthday"; // 'birthday' or 'anniversary'
  let celebrant = {
    name: "",
    department: "",
  ...
  // Celebration type button handlers
  const celebrationButtons = document.querySelectorAll(
    ".celebration-type-button"
  );
  celebrationButtons.forEach((button) => {
    button.addEventListener("click", () => {
      // Update button states
      celebrationButtons.forEach((b) => b.classList.remove("active"));
      button.classList.add("active");

      // Update mode classes
      document.body.classList.remove("birthday-mode", "anniversary-mode");
      document.body.classList.add(`${button.dataset.type}-mode`);

      // Auto-select the celebration type in the form
      if (button.dataset.type === "anniversary") {
        celebrationTypeToggle.checked = true;
        celebrationType = "anniversary";

        // Update form visibility
        birthdayFormFields.classList.add("hidden");
        anniversaryFormFields.classList.remove("hidden");
        document.querySelector(".celebration-form-title").textContent =
          "Work Anniversary Celebrant";
      } else {
        celebrationTypeToggle.checked = false;
        celebrationType = "birthday";

        // Update form visibility
        birthdayFormFields.classList.remove("hidden");
        anniversaryFormFields.classList.add("hidden");
        document.querySelector(".celebration-form-title").textContent =
          "Birthday Celebrant";
      }
      ...
      if (button.dataset.type === "anniversary") {
        headerTitle.innerHTML =
          '<i class="fas fa-award me-2"></i>Work Anniversary Bash<i class="fas fa-gift ms-2"></i>';
        headerTagline.textContent =
          "Celebrate career milestones and work anniversaries!";
      } else {
        headerTitle.innerHTML =
          '<i class="fas fa-birthday-cake me-2"></i>Birthday Bash<i class="fas fa-gift ms-2"></i>';
        headerTagline.textContent =
          "Fun games for office birthday celebrations!";
      }
    });
  });

  // Birthday Person Elements
  const toggleBirthdayPersonBtn = document.getElementById(
    "toggle-birthday-person"
  );
  const birthdayPersonForm = document.getElementById("birthday-person-form");
  const birthdayPersonDisplay = document.getElementById(
    "birthday-person-display"
  );
  const celebrantNameInput = document.getElementById("celebrant-name");
  const celebrantDepartmentSelect = document.getElementById(
    "celebrant-department"
  );
  const saveBirthdayPersonBtn = document.getElementById("save-birthday-person");
  const cancelBirthdayPersonBtn = document.getElementById(
    "cancel-birthday-person"
  );
  ...
  // Add the years selector for anniversaries
  const yearsAtCompanySelect = document.getElementById("years-at-company");
  const displayYears = document.getElementById("display-years");
  const celebrationTypeToggle = document.getElementById(
    "celebration-type-toggle"
  );
  const birthdayFormFields = document.getElementById("birthday-form-fields");
  const anniversaryFormFields = document.getElementById(
  ...
  // Update the toggle function to switch between birthday and anniversary modes
  celebrationTypeToggle.addEventListener("change", () => {
    celebrationType = celebrationTypeToggle.checked
      ? "anniversary"
      : "birthday";

    // Update form visibility based on celebration type
  ...
  // Update save celebrant function to handle both types
  function saveCelebrant() {
    ...
    // Save celebrant info
    celebrant.name = name;
    celebrant.department = department;
    celebrant.celebrationType = celebrationType;

    // Handle anniversary-specific data
    if (celebrationType === "anniversary") {
      const years = parseInt(yearsAtCompanySelect.value, 10) || 1;
      celebrant.yearsAtCompany = years;
      displayYears.textContent = `${years} ${years === 1 ? "year" : "years"}`;
    }
    ...
    // Update card name and type
    cardName.textContent = name;
    document.getElementById("celebration-type-display").textContent =
      celebrationType === "anniversary"
        ? `${celebrant.yearsAtCompany} Year Work Anniversary`
        : "Birthday";

    // Update card theme
    document.getElementById("birthday-card").className =
      celebrationType === "anniversary"
        ? "card anniversary-theme"
        : "card birthday-theme";
  ...
  async function loadGameData(game) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load data for ${game}: ${response.statusText}`
        );
      }
  ...
  function toggleBirthdayPerson() {
    if (
      birthdayPersonForm.classList.contains("hidden") &&
      birthdayPersonDisplay.classList.contains("hidden")
    ) {
      // Show form
  ...
  function shareResult() {
    ...
    if (celebrant.name) {
      if (celebrant.celebrationType === "anniversary") {
        shareText = `I scored ${userScore} points in the Anniversary Celebration game for ${celebrant.name}'s ${celebrant.yearsAtCompany}-year work anniversary on DevPlay Hub! Come join the celebration!`;
      } else {
        shareText = `I scored ${userScore} points in the Birthday Bash game for ${celebrant.name} on DevPlay Hub! Come join the celebration!`;
      }
    } else {
      shareText = `I scored ${userScore} points in the Celebration game on DevPlay Hub! Can you beat my score?`;
    }
  ...
  function addMessage() {
    ...
    if (celebrant.celebrationType === "anniversary") {
      cardMessages.classList.add("anniversary-messages");
    } else {
      cardMessages.classList.remove("anniversary-messages");
    }
  ...
  function addAnniversaryEffect() {
    // Show gold confetti for anniversaries
    const colors = ["#FFD700", "#E6BE8A", "#FFC125", "#FFDF00", "#F0E68C"];

    // Create confetti elements
    for (let i = 0; i < 120; i++) {
      const confetti = document.createElement("div");
      confetti.className = "confetti anniversary-confetti";
      confetti.style.left = Math.random() * 100 + "vw";
      confetti.style.animationDuration = Math.random() * 3 + 2 + "s";
      confetti.style.backgroundColor =
        colors[Math.floor(Math.random() * colors.length)];
  ...
  function addPartyEffect() {
    ...
    // Play appropriate celebration sound if available
    try {
      const audioFile =
        celebrant.celebrationType === "anniversary"
          ? "sounds/anniversary.mp3"
  ...
  function saveCardMessages() {
    ...
    localStorage.setItem(
      `birthday-card-${birthdayPerson.name}`,
  ...
  saveBirthdayPerson = saveCelebrant;
